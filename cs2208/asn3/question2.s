...
Prefix	LDRB	r3, [r1, #1]!			; instruction 4 : load the next byte of the string from the start to register r3
	...
	SUBGT	r3, r3, #32			; instruction 6 : if larger than #90(Z), subtract #32 from register r3 to avoid 
						; 		  small case letter
...
Suffix	LDRB	r4, [r2, #-1]!			; instruction 11: load the previous byte of the string from the end to register r4
	...
	SUBGT	r4, r4, #32			; instruction 13: if larger than #90(Z), subtract 32 from register r4 to avoid
						; 		  small case letter
	...
	MOVNE	r0, #2				; instruction 19: if the letters are not the same, put #2 in register r0, and the 
