...
Loop	LDRB	r2, [r0, #1]!			  	; instruction 3 : load the next odd index byte of the string to register r2
      ...
      ADD		r4, r4, r2				  	; instruction 8 : add the product stored in register r2 to register r4 with the sum of odd index digit
      ...
      ADD		r4, r4, r5					  ; instruction 12: add the sum of digit with both odd and even index stored in register r4 and r4 to r5
Mod		CMP		r4, #10						    ; instruction 13: mod 10 by subtraction, first compare the sum stored in register r4 with #10
      ...
      MOVNE	r0, #2						    ; instruction 18: if the remainder is not #0, put #2 in register r0, and the string code is not valid
