	...
	MOV	r4, r1				; instruction 4	: register r4 points at the begining of string1 as temporary pointer
...
Loop	SUB	r4, r1, #1			; instruction 8 : register r4 traceback 1 byte prior from register r1 for comparison
...
Start	LDRBEQ	r0, [r4], #1			; instruction 13: if current character is start of word, load the next byte from string1
						;		  pointed by temporary pointer register r4 to r0
	CMPEQ	r0, #0x74			; instruction 14: if the next byte is loaded, compare character in register r0 with 't'(0x74)
	LDRBEQ	r0, [r4], #1			; instruction 15: if current character is 't'(0x74), load the next byte from string1 pointed
	...
	LDRBEQ	r0, [r4], #1			; instruction 17: if current character is 'h'(0x68), load the next byte from string1 pointed
						;		  by temporary pointer register r4 to r0
	CMPEQ	r0, #0x65			; instruction 18: if the next byte is loaded, compare character in register r0 with 'e'(0x65)
	LDRBEQ	r0, [r4], #1			; instruction 19: if current character is 'e'(0x65), load the next byte from string1 pointed
						;		  by temporary pointer register r4 to r0
	CMPEQ	r0, #0x20			; instruction 20: if the next byte is loaded, compare character in register r0 with ' '(0x20)
	SUBEQ	r1, r4, #1			; instruction 21: if current character is ' '(0x20), the pattern of single word "the" is
						;		  matched in string1, update register r1 to 1 byte prior of temporary pointer 
						;		  register r4 to include the last ' ' to store in string2
Skip	CMP	r4, r2				; instruction 22: if Z flag is cleared during any of the comparison processes or skipped, 
	...
	STRB	r0, [r3], #1			; instruction 25: store character in register r0 to the next available byte of string2 
						;		  pointed by register r3
	CMP	r1, r2				; instruction 26: compare the address of next loading byte stored in register r1 with the
						;		  address of the ending of string1 stored in register r2
